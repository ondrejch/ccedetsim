...
double D	= 0.01;//13;		// sensor depth [cm]
...
void ccesensor2d(long Nneutrons=10, char* outfname="mydet2d.root", char* parfname="mydet2d.par") {
  cout << "Running Sensor2D with "<<Nneutrons<<" neutrons" <<endl;
...
// create and initialize objects we need
  ...
  if(debug>0) nt = new TNtuple("nt","Detailed event info","x:y:angle:cosangle:edep:cce");
  
  double x;   	// neutron interaction X coordinate
  double y;   	// neutron interaction Y coordinate
...
int main(int argc, char *argv[]) {
  ...
  ccesensor2d(n, outfname, parfname);  
  fbeamatt->Write();
  fcce->Write();
  hcce->Write();
  if(nt) nt->Write();
