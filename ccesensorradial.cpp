/* ROOT macro to 2D case detector simualtion with CCE sum		*
 *  g++ ccesensorradial.C -o ccesensorradial.exe `root-config --cflags --libs`
 *      Ondrej, April 6 2013                                            */
#include <vector>
#include <iostream>
#include <fstream>
...
Format: plain ASCII file with the following entries, NUMBERS ONLY, DIFFERENT FOR 2D!!!
<W>	<R> 	<Nsigma>
<Efield>	<mu_e>	<t_e>	<mu_h>	<t_h>
<Nccebins>	<ccemin>	<ccemax>					*/

double W 	= 0.01;//178; 	// sensor length [cm]
double R	= 0.01;//13;	// sensor radius [cm]
...
double t_e  = 1e-6;		// electron collection time = 10^-6 s
...
int Nccebins = (ccemax-ccemin)*2e4;		// bins for CCE sums
...
void initPars(char* parfname="mydet2d.par") {
  ifstream fin;
  fin.open(parfname);
  ...
  fin.close();
  cout << "* Detector parameters, 2D case *" << endl;
  cout << "* W = " << W << " cm" <<endl << "* R = " << R << " cm" <<endl;
...
void ccesensorradial(long Nneutrons=10, char* outfname="mydet2d.root", char* parfname="mydet2d.par") {
  cout << "Running SensorRadial with "<<Nneutrons<<" neutrons" <<endl;
...
// create and initialize objects we need
  ...
  hcce = new TH1D("hcce",Form("Sensor2D amplitudes, det W = %f cm, D = %f cm",W,D), Nccebins, ccemin, ccemax);
  ...
  if(debug>0) nt = new TNtuple("nt","Detailed event info","x:y:z:r:angle:cosangle:edep:cce");
  
  double x;   	// neutron interaction X coordinate
  double y;   	// neutron interaction Y coordinate \__ radial dimesions [y,z]
  double z;   	// neutron interaction Z coordinate / 
  double angle;	// decay angle of triton w.r.t X coordinate
  for (int i=1; i<=Nneutrons; i++) { 	// fire neutrons!
    double ccesum = 0, edep = 0;
    x = fbeamatt->GetRandom(); 		// could be sped up by rndgen->Exp(tau)
    do {
      y = rndgen->Rndm() *2.*D - D; 	// [-D .. D]	
      z = rndgen->Rndm() *2.*D - D; 	// [-D .. D]	
    } while (x*x + y*y) > D*D;
    angle = rndgen->Rndm()*TMath::TwoPi(); 	// random angle
    const double cosangle = TMath::Cos(angle); 	// avoid recalculating cos 
    const double sinangle = TMath::Sin(angle); 	// avoid recalculating sin
    // process triton response
    for(int j=0; j<maxT; j++) { 
      double xcosr = x + braggT[j].r * cosangle;
      double ysinr = y + braggT[j].r * sinangle;
      if( xcosr>0 && xcosr<W && ysinr>0 && ysinr<D ) 
    ...
    for(int j=0; j<maxA; j++) { 
      ...
      double ysinr = y - braggA[j].r * sinangle;
      if( xcosr>0 && xcosr<W && ysinr>0 && ysinr<D ) 
    ...
    hcce->Fill(ccesum);
    if(debug>0) nt->Fill(x,y,angle,cosangle,edep,ccesum);
...
int main(int argc, char *argv[]) {
  long n = 10;
  char outfname[200] = "mydet2d.root";
  char parfname[200] = "mydet2d.par";
  ...
  ccesensorradial(n, outfname, parfname);  
  fbeamatt->Write();
  fcce->Write();
  hcce->Write();
  if(nt) nt->Write();
